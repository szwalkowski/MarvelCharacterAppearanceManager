...
describe("Page model test based on downloaded html", function () {
    ...
    it("parse html for Black Cat that does have sup link for real name", function () {
        ...
        const pageWindow = new JSDOM(page).window;
        const rhinoPage = new CharacterPageModel("https://marvel.fandom.com", pageWindow);
        assert.equal(rhinoPage.getId(), "Felicia Hardy (Earth-616)");
        assert.equal(rhinoPage.getRealName(), "Felicia Sara Hardy");
        assert.equal(rhinoPage.getCurrentAlias(), "Black Cat");
        assert.equal(rhinoPage.getUniverse(), "Earth-616");
        assert.equal(rhinoPage.getImage(), "https://vignette.wikia.nocookie.net/marveldatabase/images/f/fd/Amazing_Spider-Man_Vol_5_1_Crain_Exclusive_Black_Cat_Variant.jpg/revision/latest/scale-to-width-down/323?cb=20180723081925");
    ...
    it("parse html for Hammerhead that does not have link for current alias", function () {
        ...
        const pageWindow = new JSDOM(page).window;
        const rhinoPage = new CharacterPageModel("https://marvel.fandom.com", pageWindow);
        assert.equal(rhinoPage.getId(), "Hammerhead (Joseph) (Earth-616)");
        assert.equal(rhinoPage.getRealName(), "Joseph(last name unrevealed)");
        assert.equal(rhinoPage.getCurrentAlias(), "Hammerhead");
        assert.equal(rhinoPage.getUniverse(), "Earth-616");
        assert.equal(rhinoPage.getImage(), "https://vignette.wikia.nocookie.net/marveldatabase/images/1/15/Amazing_Spider-Man_Vol_3_17.1_Textless.jpg/revision/latest/scale-to-width-down/316?cb=20150119193829");
    ...
    it("parse html for Klaw that does not have weird real name", function () {
        ...
        const pageWindow = new JSDOM(page).window;
        const rhinoPage = new CharacterPageModel("https://marvel.fandom.com", pageWindow);
        assert.equal(rhinoPage.getId(), "Ulysses Klaw (Earth-616)");
        assert.equal(rhinoPage.getRealName(), "Ulysses S. Klaw");
        assert.equal(rhinoPage.getCurrentAlias(), "Klaw");
        assert.equal(rhinoPage.getUniverse(), "Earth-616");
        assert.equal(rhinoPage.getImage(), "https://vignette.wikia.nocookie.net/marveldatabase/images/1/10/Ulysses_Klaw_%28Earth-616%29_from_Superior_Carnage_Vol_1_1_001.jpg/revision/latest/scale-to-width-down/325?cb=20130718233151");
